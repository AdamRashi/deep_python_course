...
class TestFilterLines(unittest.TestCase):
    ...
    def test_several_matches(
        ...
        expected_result = ["Do you want an apple", "Sweet one", "And a banana"]
        result = list(filter_lines(TEST_FILE_NAME, filter_words))
        self.assertEqual(result, expected_result)

        # проверим с файловым объектом
        with open(TEST_FILE_NAME, "r") as file_obj:
            result = list(filter_lines(file_obj, filter_words))
            self.assertEqual(result, expected_result)
    ...
    def test_empty_file(self):
        ...
        with open(file_name, "r", encoding="utf-8") as file_obj:
            results = list(filter_lines(file_obj, filter_words))
            self.assertEqual(results, [])
    ...
    def test_no_filter_words(self):
        ...
        # проверим с файловым объектом
        with open(TEST_FILE_NAME, "r") as file_obj:
            result = list(filter_lines(file_obj, []))
            self.assertEqual(result, [])
